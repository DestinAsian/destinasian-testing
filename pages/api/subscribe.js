...
export default async function handler(req, res) {
  ...
  if (rateLimit.limited) {
    return res.status(429).json({ message: 'Too much requests, try again later.' })
  }
  ...
  if (!process.env.MAILERLITE_GROUP_ID || !process.env.MAILERLITE_API_KEY) {
    return res.status(500).json({ message: 'MailerLite not configured yet' })
  }
  ...
  try {
    ...
    if (!createRes.ok) {
      return res.status(createRes.status).json({
        message: subscriber?.error || 'Failed to subscribe',
      })
    }

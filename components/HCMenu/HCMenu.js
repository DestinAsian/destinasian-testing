...
export default function HCMenu(customClassName) {
  const menuVariables = {
    first: 100,
    hcHeaderLocation: HC_LOCATION,
  }

  const { data, isLoading } = useSWRGraphQL(
    HC_LOCATION ? ['hc-menu', HC_LOCATION] : null,
    GetHCMenu,
    menuVariables,
  )

  const hcMenu = data?.hcHeaderMenuItems?.nodes ?? []
              ...
              <li key={id} id={id} className={cx('menu-item', cssClasses)}>
                {path && (
                  <Link
                    ...
                    className={cx(
                      'title',
                      customClassName?.customClassName === 'dark-color'
                        ? 'title-dark'
                        : '',
                    )}
                  >

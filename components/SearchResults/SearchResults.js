...
const CONTENT_META = {
  ...
  ReadersChoiceAwards: (node) => ({
    label: 'Readers Choice Award',
    url: node.uri,
  }),
  LuxeLists: (node) => ({
    label: 'Luxe List',
  ...
  LuxuryTravels: (node) => ({
    label: node.parent?.node?.title,
    url: node.parent?.node?.uri,
  }),
  HonorsCircles: (node) => ({
    label: 'Honoers Circle',
    url: node.uri,
  }),
}
    ...
    <div className={cx('component')}>
      ...
      {results.length > 0 && hasMore && (
        <div className="mx-auto my-0 flex max-w-[100vw] justify-center md:max-w-[700px]">
          ...
          >
            {isFetchingMore ? 'Loading...' : 'Search Older Stories'}
          </button>
        </div>
      )}
    </div>

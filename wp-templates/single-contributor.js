...
export default function SingleContributor(props) {
  ...
  useEffect(() => {
    const storedPassword = Cookies.get('contributorPassword')
    if (
      storedPassword &&
      storedPassword === props?.data?.contributor?.passwordProtected?.password
    ) {
      setIsAuthenticated(true)
    }
  }, [props?.data?.contributor?.passwordProtected?.password])
  ...
  const handlePasswordSubmit = (e) => {
    ...
    if (enteredPassword === passwordProtected?.password) {
      setIsAuthenticated(true)
      Cookies.set('contributorPassword', enteredPassword, { expires: 1 }) // Set cookie to expire in 1 day
    } else {
